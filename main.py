...
# PARSING CURRENCY
# req_value = requests.get(
#     url = "https://cs.money/ru/csgo/trade/?utm_source=google&utm_medium=cpc&utm_campaign=Search_Brand_RU_Desktop_Act&"
#           "network=g&utm_term=%2Bcsmoney&utm_content=ads_1&"
#           "gclid=Cj0KCQiArt6PBhCoARIsAMF5wajjdx0O5pOFaBfDgZfpGs0Qj3xXngZ2qIjtV-z-P_JM5iTn2LLUjI0aApW8EALw_wcB&"
#           "sort=price&order=asc&minPrice=10000&isMarket=false&hasRareFloat=false&hasRareStickers=false&"
#           "hasRarePattern=false&hasTradeLock=false&hasTradeLock=true",
#     headers = headers_
# )
# form = req_value.text
# Currency = BeautifulSoup(form, "lxml")
#
# USD = float(Currency.find(class_="styles_input__1osOB").get("value"))/10000


#PARSE SKINS DATA
# request =requests.get(
#     url="https://inventories.cs.money/5.0/load_bots_inventory/730?hasRareFloat=false&hasRareStickers=true&"
#         "hasTradeLock=false&hasTradeLock=true&isMarket=false&limit=60&minPrice=16.12910483903629&offset=0&"
#         "order=asc&priceWithBonus=30&sort=price&tradeLockDays=1&tradeLockDays=2&tradeLockDays=3&"
#         "tradeLockDays=4&tradeLockDays=5&tradeLockDays=6&tradeLockDays=7&tradeLockDays=0&withStack=true",
#     headers=headers_
# )
# data = request.json()
# data_ = data["items"]
...
#     for item in data_:
#         file.writelines(["Name: ", str(item["fullName"]), '\n',
#                         "Price: " , str("%.2f" % (item["price"]*USD)), '\n',
#                         "SteamID: ", str(item["steamId"]), "\n"])
#     file.close()
...
#         f"name={name.replace(' ', '%20')}&offset=0&order=asc&priceWithBonus=30&"
...
# listing = data.find_all(class_="normal_price")
# names = data.find_all(class_="market_listing_item_name")

# prices_list = []
# names_list = []
# for item in listing[1::2]:
#     price = item.get_text()
...
#
# for i in range(10):
#     print(f"Name: {names_list[i]}\n"
#           f"Steam price: {prices_list[i]}\n\n")


# req[req.find("data-hash-name=\\")+16:req.find(">\r\n\t\t\t\t<img id=\\")]


#
# print(request, '\n')

# with open("steam.txt", "w", encoding="utf-16") as file:
...
#           f"Name: {names_list[i]}\n"
#           f"Steam price: {prices_list[i]}\n"
#           f"Steam link: {link_list[i]}\n"
#           f"CS.money price: {cs_prices_list[i]}\n\n")
